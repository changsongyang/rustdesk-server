...
impl UserManager {
    ...
    pub async fn update_user(&self, id: &str, request: UserUpdateRequest) -> Result<UserInfo, UserError> {
        // Get current user
    ...
    pub async fn list_users(&self, organization_id: Option<&str>) -> Result<Vec<UserInfo>, UserError> {
    ...
    pub async fn login(&self, request: UserLoginRequest) -> Result<UserLoginResponse, UserError> {
        ...
        let user = UserInfo {
            id: row.try_get("id").map_err(|_| UserError::InvalidCredentials)?,
            username: row.try_get("username").map_err(|_| UserError::InvalidCredentials)?,
            email: row.try_get("email").map_err(|_| UserError::InvalidCredentials)?,
            role: row.try_get("role").map_err(|_| UserError::InvalidCredentials)?,
            organization_id: row.try_get("organization_id").ok(),
        };
